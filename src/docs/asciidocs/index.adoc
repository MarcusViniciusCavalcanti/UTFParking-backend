...
IMPORTANT:: Fique atento para o tipo de lista de recurso em cada url de páginação, este é apenas um exemplo
cada lista de recurso será representando por um tipo especifico consulte cada sessão para saber qual.

### Navegação entre páginas
Cada url que retorna uma lista de recursos, pode receber parâmetros que será utilizado para definir a quantidade
de recursos exibidos ou qual página será recuperada
